# Unweighted Un-Directed Graph using an Adjacency Matrix

class UnweightedUndirectedGraphMatrix:
    def __init__(self, size=5):
        """Initializes an adjacency matrix for an undirected graph."""
        self.size = size
        self.matrix = [[0] * size for _ in range(size)]  # Create NxN matrix
    ...
    def add_edge(self, u, v):
        ...
        if u >= self.size or v >= self.size:
            ...
            return
        self.matrix[u][v] = 1  # u → v
        self.matrix[v][u] = 1  # v → u (undirected)
    ...
    def remove_edge(self, u, v):
        ...
        if u >= self.size or v >= self.size:
            print(f"Error: Node {max(u, v)} does not exist.")
            return
        self.matrix[u][v] = 0  # Remove edge u → v
        self.matrix[v][u] = 0  # Remove edge v → u (undirected)
...
# Example Usage:
graph = UnweightedUndirectedGraphMatrix(6)  # Create a graph with 6 vertices
